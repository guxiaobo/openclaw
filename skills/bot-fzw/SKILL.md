...
# 自动fallback：先走requests，失败后自动切browser再试一次
venv/bin/python3 fzw.py --card 110101199001011234 --auto-fallback
```
...
- **自动fallback**：支持 `--auto-fallback`，先走 requests；若出现典型链路/WAF失败，再自动切换 browser 重试一次
